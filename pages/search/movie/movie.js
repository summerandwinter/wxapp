...
Page({
  ...
  search: function (e) {
    ...
    Movie.search(data, function (data) {
      console.log(data);
  ...
  loadMore: function () {
    ...
    if (!that.data.collections.hidden) {
      ...
      Movie.toplist(data, function (data) {
        console.log(data);
        var start = data.start;
        var total = data.total;
        var count = data.count;
        ...
        that.setData({
          ...
          'isLoading': false,
          'loading.hidden': true,
          'collections.list': that.data.collections.list.concat(data.subjects),
      ...
      Movie.search(data, function (data) {
        ...
        if (data.subjects.length > 0) {
          ...
          that.setData({
            ...
            'loading.hidden': true,
            'collections.hidden': true,
            'info.list': that.data.info.list.concat(data.subjects),
            'info.hidden': false
          })
        } else {
          that.setData({
  ...
  initData: function () {
    ...
    Movie.toplist(data, function (data) {
      ...
      that.setData({
        ...
        'hasMore': hasMore,
        'loading.hidden': true,
        'collections.list': data.subjects,
        'info.hidden': true,
        'collections.hidden': false
      })
    }, function (res) { })
